...
$user_id = $this->input->get('merchant_id'); ?>
<div class="wrapper">
        ...
        <div class="btn-group dropup">
            ...
            <div class="dropdown-menu">
                <a class="dropdown-item"
                   href="<?= base_url("complainStatus?merchant_id=$user_id&status=1") ?>">Pending</a>
                <div class="dropdown-divider"></div>
                <a class="dropdown-item"
                   href="<?= base_url("complainStatus?merchant_id=$user_id&status=0") ?>">Resolve</a>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function () {
        ...
        let user_id = <?= $user_id?>;
        $.ajax({
            url: "<?= base_url('getMsgForMerchant') ?>",
            type: "POST",
            ...
            complete: function () {
                ...
                $(".ddd").animate({scrollTop: $('.ddd').get(0).scrollHeight}, 1);
        ...
        $('.send').click(function () {
            ...
            let sender_id = '3';
            $.ajax({
                url: "<?= base_url('sendMsgMerchant') ?>",
                type: "POST",
                data: {
                    message: msg,
                    merchant_id: complain_id,
                    sender_id: sender_id
                },
                beforeSend: function () {
